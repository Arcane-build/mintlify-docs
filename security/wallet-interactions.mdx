...
| Sign transaction (`signTransaction` / `sendTransaction`) | ✅ Yes | Only on explicit user action (place bet, claim, manage quick-bet session) |
| Embedded wallet creation | ❌ No | Disabled (`createOnLogin: 'off'`) |
| Privy auto-approve / blind signing | ❌ No | Not used. Repeated bets use an explicit, user-funded quick-bet session instead (see below) |
...
<AccordionGroup>
  ...
  <Accordion title="Transaction signing (on-chain)">
    Used for `place_bet`, `claim`, and quick-bet session management. Each is a
    standard Solana transaction the user reviews in their wallet.
  </Accordion>
  <Accordion title="Quick-bet delegate session">
...
| Quick-bet escrow (delegate session) | ✅ Optional | A user-funded, capped, revocable on-chain escrow — see above |
...
explicit transaction signatures for each bet or claim. Stakes are native SOL that
move only through documented instructions, the optional quick-bet delegate is
