...
<Steps>
  ...
  <Step title="User connects a Solana wallet">
    ...
    this step. [INSERT screenshot of the connect prompt].
  </Step>
  ...
  <Step title="(Optional) Quick-bet session">
    To avoid re-prompting on every bet, the user can sign once to create an
    on-chain quick-bet session funded with a spend cap they choose. Bets are then
...
<Steps>
  ...
  <Step title="App builds a Solana transaction">
    ...
    for a quick-bet session) using the program IDL. Parameters match the displayed
    action.
  </Step>
  <Step title="Wallet shows the transaction">
    The Solana wallet displays the instruction and accounts for review. [INSERT
...
| Bet | Market + outcome + stake | One on-chain transaction (`place_bet`) |
| Claim | Resolved market + payout | One on-chain transaction (`claim`) |

## Expected result after signing

After a bet, the user's SOL moves into the market vault and their stake on the
